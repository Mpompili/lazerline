class GameView {
    ...
    compPingDirections() {
        ...
        //up 
        for (let i = y - 5; i >= 0; i -= 1){
            pong.up = y - i - 5;
            if (this.compIsCollision(x, i)) {
        ...
        //left
        for (let i = x - 5 ; i >= 0; i -= 1){
            pong.left = x - i - 5; 
            if (this.compIsCollision(i, y)) {
    ...
    gameOver(win) {
        ...
        console.log(this.comp.location);
        console.log(win);
